...
def get_rife_model():
    ...
    if rife_model is None:
        ...
        if settings.enable_torch_compile and torch.cuda.is_available():
            rife_model.flownet = torch.compile(rife_model.flownet, mode="reduce-overhead")
            
    return rife_model

def fallback_static_interpolation(frame1: VideoFrame, _frame2: VideoFrame, count: int = 5) -> List[VideoFrame]:
    frames = []
...
def interpolate_frames(frame1: VideoFrame, frame2: VideoFrame, count: int = 5) -> List[VideoFrame]:
    try:
        ...
        I0 = torch.from_numpy(img1.transpose(2, 0, 1)).to(device, non_blocking=True).unsqueeze(0) / 255.0
        I1 = torch.from_numpy(img2.transpose(2, 0, 1)).to(device, non_blocking=True).unsqueeze(0) / 255.0
        ...
        def make_inference(I0_t, I1_t, n):
            middle = model.inference(I0_t, I1_t, 1.0)
            if n == 1:
                return [middle]
        ...
        if count > 1:
            with torch.no_grad():
                intermediate_tensors = make_inference(I0_padded, I1_padded, count - 1)
        else:
        ...
        def tensor_to_frame(tensor_padded):
            ...
            img_np = (tensor[0].clamp(0.0, 1.0) * 255.0).byte().cpu().numpy().transpose(1, 2, 0)
