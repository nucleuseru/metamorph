...
def get_pipeline():
    ...
    if pipe is None:
        ...
        pipe = Flux2KleinKVPipeline.from_pretrained(settings.model_id, vae=vae, torch_dtype=dtype)
...
def run_inference(frame, session_data: Dict[str, Any]):
    try:
        ...
        if "prompt_embeds" not in session_data:
            ...
            if hasattr(local_pipe, "text_encoder") and local_pipe.text_encoder is not None:
        ...
        if "uploaded_images_pil" not in session_data:
            ...
            for img_bytes in session_data.get("images", []):
                try:
                    uploaded_images.append(Image.open(BytesIO(img_bytes)).convert("RGB"))
